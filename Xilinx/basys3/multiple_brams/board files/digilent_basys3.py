...
from litex.soc.cores.ip.AXI_GPIO import gpio_axi
from litex.soc.cores.ip.axi_ram import axi_ram
from litex.soc.cores.ip.axi_ram import axi_ram_1
...
class BaseSoC(SoCCore):
    def __init__(self, sys_clk_freq=int(75e6), with_led_chaser=True, axiram = False, axigpio = False, with_video_terminal=False, **kwargs):
...
#
        ...
        if axiram:
            self.submodules.axi_ram_1 = axi_ram_1(platform, pads=None)
            self.bus.add_slave(name="axi_ram_1", slave=self.axi_ram_1.bus, region=SoCRegion(
                origin = 0x50000100,
                size   = 128,
                cached = True,
        ))
...
def main():
    ...
    parser.add_argument("--axigpio",            action="store_true",     help="GPIO IP integrated by RS" )
    parser.add_argument("--axiram",            action="store_true",     help="AXI_RAM IP integrated by RS" )
